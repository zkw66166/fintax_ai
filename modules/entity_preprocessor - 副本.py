...
# 资产负债表用"货币资金"（汇总），科目余额表才有"银行存款"、"库存现金"等明细科目
_ACCOUNT_SPECIFIC_NAMES = [
    '银行存款', '库存现金', '其他货币资金',
    '主营业务收入', '主营业务成本', '其他业务收入', '其他业务成本',
    '本年利润', '利润分配',
    '现金余额',  # "现金余额"指科目级明细，资产负债表用"货币资金"
]
...
_BALANCE_SHEET_ITEMS_UNIQUE = [
    '交易性金融资产', '衍生金融资产', '应收账款融资', '合同资产',
    '持有待售资产', '一年内到期的非流动资产', '债权投资', '其他债权投资',
    '长期应收款', '其他权益工具投资', '其他非流动金融资产', '投资性房地产',
    '使用权资产', '商誉', '递延所得税资产',
    '交易性金融负债', '衍生金融负债', '合同负债', '持有待售负债',
    '一年内到期的非流动负债', '应付债券', '租赁负债', '预计负债',
    '递延所得税负债', '库存股', '其他综合收益', '专项储备',
    '短期投资', '应收股利', '应收利息', '原材料', '在产品',
    '库存商品', '周转材料', '长期债券投资', '固定资产原价',
    '累计折旧', '固定资产账面价值', '工程物资', '固定资产清理',
    '应付利息', '应付利润',
]
...
# "所得税"需要排除"所得税费用"（利润表项目）的情况，单独处理
_EIT_KEYWORD_SUODESHUI = '所得税'
_EIT_SUODESHUI_EXCLUSIONS = ['所得税费用']


def _has_eit_keyword(query: str) -> bool:
    """检查查询中是否包含EIT域关键词，'所得税'需排除'所得税费用'"""
    for kw in _EIT_KEYWORDS:
        if kw in query:
            return True
    # 单独检查"所得税"：排除"所得税费用"
    if _EIT_KEYWORD_SUODESHUI in query:
        # 检查所有出现位置，是否都是"所得税费用"的一部分
        idx = 0
        while True:
            pos = query.find(_EIT_KEYWORD_SUODESHUI, idx)
            if pos == -1:
                break
            # 检查是否被排除词覆盖
            excluded = False
            for excl in _EIT_SUODESHUI_EXCLUSIONS:
