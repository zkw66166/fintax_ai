# 企业税务与财务 NL2SQL 数据模型与受控生成方案文档（最终自洽稿）  
**版本**：**v1.2** | **最后更新**：2026-02-13  

> v1.2 变更摘要（相对 v1.1）  
> 1) **跨两类纳税人总体对比允许 UNION**（推荐），不再强制 JOIN。  
> 2) 允许对 **6.3、7、8** 做同步修订，彻底移除与“拆分视图/取消公共视图”的矛盾表述。  
> 3) **细化 6.2 Prompt 设计（方案A：两阶段受控生成 + 动态 Schema 注入）**：补齐到“可落地实现”的颗粒度，并给出你提供的 **6 套分域 System Prompt（终版）** 的适配版（含必要注释建议）。  
> 4) 同步更新与 v1.1 冲突的地方：删除对 `vw_vat_return_full`、`vw_vat_common_indicators` 的依赖，统一改为 `vw_vat_return_general` / `vw_vat_return_small` 编排。  
