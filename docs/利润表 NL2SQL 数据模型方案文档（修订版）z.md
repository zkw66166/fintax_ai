...
1. **利润表数据库设计不符合业务需求**：数据库中利润表设计不符合业务需求，因为现有设计是将企业会计准则和小企业会计准则合并为一个宽表，但后续利润表项目可能经常变动，且需要增加更多会计准则和会计制度的利润表，频繁修改宽表难以维护。
2. **目标**：数只修改利润表相关的表设计，但保持利润表相关视图结构不变，从而实现原脚本中对视图的引用不变。
...
('22行', 'profit_before_tax_current', 'CAS', 3),
...
('30行', 'profit_before_tax_current', 'SAS', 3),
