...
# ============ 可重载的独立函数（供 data_update_service 调用） ============

def seed_vat_synonyms(db_path=None):
    """重载增值税同义词表"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_eit_synonyms(db_path=None):
    """重载企业所得税同义词表"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_account_synonyms(db_path=None):
    """重载科目余额同义词表"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_balance_sheet_data(db_path=None):
    """重载资产负债表数据"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_income_statement_data(db_path=None):
    """重载利润表数据"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_cash_flow_data(db_path=None):
    """重载现金流量表数据"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_invoice_data(db_path=None):
    """重载发票数据"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def seed_financial_metrics_data(db_path=None):
    """重载财务指标数据"""
    db_path = db_path or str(DB_PATH)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
